...
public class TreatmentControllerTests
{
    ...
    {
        // Arrange
        var treatments = new List<TreatmentDto> { new(), new() };

        _mediatorMock
            .Setup(m => m.Send(new GetTreatmentsQuery(), CancellationToken.None))
        ...
        var value = okResult?.Value as List<TreatmentDto>;
        value.Should().HaveCount(2);
        value.Should().BeEquivalentTo(treatments);
